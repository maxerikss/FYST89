...
\begin{subequations}\begin{align}
    ...
    \text{Inverses:}&& R R^{-1} =1 &\implies \mathscr{D}(R) \mathscr{D}^{-1}(R) = 1 \\[-0.2\baselineskip]
...
we also find
\begin{equation}
    \bkop{\vec{x}'}{\vec{L}^2}{\alpha} = -\hbar^2 \ab [ \frac{1}{\sin^2\theta} \partial_\phi^2 + \frac{1}{\sin\theta} \partial_\theta (\sin\theta \partial_\theta) ]\bk{\vec{x}'}{\alpha},
\end{equation}
...
\begin{equation}
    \bk{\vec{x}'}{n,l,m} = R_{nl}(r) Y_l^m(\theta, \phi),
\end{equation}
which is a direct consequence of the rotational invariance of the system. For a spherical symmetric system $H$ commutes with $L_z$ and $\vec{L}^2$ and the energy eigenkets are thus eigenkets for $L_z$ and $\vec{L}^2$ also. Since the angular dependence is the same for all systems with spherical symmetry we can isolate this using the direction eigenket $\k{\hat{\vec{n}}}$, and write
\begin{equation}
    \bk{\hat{\vec{n}}}{l,m} = Y_l^m(\theta,\phi) = Y_l^m (\hat{\vec{n}}).
\end{equation}
We can think of $Y_l^m(\theta,\phi)$ as the amplitude of the state characterized by $l,m$ found in the direction of $\hat{\vec{n}}$ specified by the angles $\theta,\phi$. From eigenvalue equations such as 
\begin{equation}
    L_z\k{l,m} = m\hbar \k{l,m}
\end{equation}
we can infer the dependence on $Y_l^m(\theta, \phi)$. Multiplying by $\b{\hat{\vec{n}}}$ we find
\begin{equation}
    \bkop{\hat{\vec{n}}}{L_z}{l,m} = -i\hbar \partial_\phi \bk{\hat{\vec{n}}}{l,m} = m\hbar \bk{\hat{\vec{n}}}{l,m},
\end{equation}
which is
\begin{equation}
    -i\hbar \partial\phi Y_l^m(\theta, \phi) = m\hbar Y_l^m(\theta,\phi),
\end{equation}
and thus the dependence on $\phi$ of $Y_l^m(\theta,\phi)$ must be $e^{im\phi}$. Similarly, using the eigenvalue equation
\begin{equation}
    \vec{L}^2 \k{l,m} = l(l+1)\hbar^2\k{l,m}
\end{equation}
we have
\begin{equation}
    \bkop{\hat{\vec{n}}}{\vec{L}^2}{l,m} = l(l+1)\hbar^2 \bk{\hat{\vec{n}}}{l,m} = -\hbar^2\ab[ \frac{1}{\sin^2\theta} \partial_\phi^2 + \frac{1}{\sin\theta}\partial_\theta (\sin\theta \partial_\theta)] \bk{\hat{\vec{n}}}{l,m},
\end{equation}
which is equivalent to 
\begin{equation}
    \ab[ \frac{1}{\sin\theta} \partial_\theta (\sin_\theta\partial\theta) + \frac{1}{\sin^2\theta} \partial_\phi^2 + l(l+1) ]Y_l^m(\theta,\phi) = 0.
\end{equation}
The orthogonality relation
\begin{equation}
    \bk{l',m'}{l,m} = \delta_{ll'}\delta_{mm'}
\end{equation}
is equivalent to 
\begin{equation}
...
which leads to 
\begin{equation}
    -i \hbar e^{i\phi} \ab[ i\partial_\phi - \cot\theta \partial_\phi ]\bk{\hat{\vec{n}}}{l,l} = 0,
\end{equation}
and this partial differential equation is satisfied by
\begin{equation}
    \bk{\hat{\vec{n}}}{l,l} = Y_l^l(\theta, \phi) = c_l e^{il\phi} \sin^l(\theta),
\end{equation}
with $c_l$ the renormalization constant
\begin{equation}
    c_l = \ab[\frac{(-1)^l}{2^l l!}] \sqrt{\frac{(2l+1)(2l)!}{4\pi}}.
\end{equation}
From this we can obtain subsequent $Y_l^m(\theta,\phi)$, which for $m\geq 0$ is
\begin{equation}
...
and we define
\begin{equation}
    Y_l^{-m} = (-1)^m [Y_l^m(\theta,\phi)]^*.
\end{equation}
Note here that we cannot have $l$ or $m$ be half-integer valued since this would give us a multivalued function since we acquire a minus sign under a $2\pi$ rotation.


\subsubsection{Spherical Harmonics as Rotation Matrices}
We can find a general direction eigenket by applying a rotation $\mathscr{D}(R)$ to $\k{\hat{\vec{z}}}$ such that
\begin{equation}
    \k{\hat{\vec{n}}} = \mathscr{D}(R)\k{\hat{\vec{z}}}.
\end{equation}
...
\begin{equation}
    \mathscr{D}(R) = \mathscr{D}(\alpha = \phi, \beta=\theta, \gamma = 0).
\end{equation}
Inserting the completeness relation in the rotation we find
\begin{equation}
    \k{\hat{\vec{n}}} = \sum_l \sum_m \mathscr{R} = \k{l,m}\bk{l,m}{\hat{\vec{z}}}.
\end{equation}
We find that $\k{\hat{\vec{n}}}$ expanded in $\k{l,m}$ contains all possible $l$. We now multiply by $\b{l,m'}$ on the left and find
\begin{align}
    \bk{l,m'}{\hat{\vec{n}}} = \sum_m \mathscr{D}_{mm'}^{(l)} (\phi,\theta, 0)\bk{l,m}{\hat{\vec{z}}},
\end{align}
and we see that $\bk{l,m}{\hat{\vec{z}}} = Y_l^(m*)(0,\phi)$ where $\phi$ is undetermined. Using this we find
\begin{equation}
    Y_l^{m**}(\theta,\phi) = \sqrt{\frac{2l+1}{4\pi}} \mathscr{D}_{m'0}^{(l)}(\phi,\theta,0)
\end{equation}
or
\begin{equation}
...
We consider the state that is a direct product state of the position and spin kets $\k{\vec{x}', \updownarrow} = \k{\vec{x}'} \otimes \k{\updownarrow}$, where operators in the space spanned by the position kets commute by the operators in the space spanned by the spin-space. The rotation operator will still be $\exp(i \vec{J}\cdot\hat{\vec{n}} \phi / \hbar)$, but angular momentum, the generator of rotation, will be
\begin{equation}
    \vec{J} = \vec{L} + \vec{S} = \vec{L} \otimes 1 + 1 \otimes \vec{S}.
\end{equation}
Since $\comm{\vec{L}}{\vec{S}}=0$ we can write
\begin{equation}
    \mathscr{D}(R) = \mathscr{D}^{(\text{orb})} \otimes \mathscr{D}^{(\text{spin})} = \exp\ab(- \frac{i \vec{L} \cdot\hat{\vec{n}}}{\hbar} \phi) \otimes \exp\ab(- \frac{i \vec{S} \cdot\hat{\vec{n}}}{\hbar} \phi).
\end{equation}
We can write the wave function for a spin particle as 
\begin{equation}
    \bk{\vec{x}', \updownarrow}{\alpha} = \psi_\updownarrow(\vec{x}')
\end{equation}
or as a column matrix
\begin{equation}
    \begin{pmatrix}
        \psi_\uparrow(\vec{x}')\\
        \psi_\downarrow(\vec{x}')
    \end{pmatrix}.
\end{equation}
We might change the representation of the space part to be the eigenket $\k{n,l,m}$.
...
\begin{equation}
    \vec{S} = \vec{S}_1 + \vec{S}_2 = \vec{S}_1 \otimes 1 + 1 \otimes \vec{S}_2,
\end{equation}
...
\begin{align}
    &\vec{S}^2 = (\vec{S}_1 + \vec{S}_2)^2 : s(s+1)\hbar^2\\
    &S_z = S_{1z} + S_{2z} : m \hbar\\
    &S_{1z} : m_1\hbar\\
    &S_{2z} : m_2\hbar.
\end{align}

We can expand the ket corresponding of an arbitrary spin of the two particle system by either eigenkets to $\vec{S}^2$ and $S_z$ or eigenkets to $S_{1z}$ and $S_{2z}$:
\begin{enumerate}
    \item Expanding in the $\{m_1, m_2\}$ basis gives the possible kets
            \begin{equation}
                \k{\uparrow \uparrow}, \k{\uparrow\downarrow}, \k{\downarrow\uparrow}, \text{and } \k{\downarrow\downarrow}.
            \end{equation}
    \item Expanding in the $\{s, m\}$ basis (also called the triplet-singlet representation) gives the possible kets ($\k{s, m}$)
            \begin{equation}
                \k{0, 0}, \k{1,-1}, \k{1,0}, \text{and } \k{1,1}.
            \end{equation}
\end{enumerate}
The relationship between these two bases are 
\begin{align}
    \k{0,0}     &= \frac{1}{\sqrt{2}} (\k{\uparrow\downarrow} - \k{\downarrow\uparrow})\\
    \k{1,-1}    &= \k{\downarrow\downarrow}\\
    \k{1,0}     &= \frac{1}{\sqrt{2}} (\k{\uparrow\downarrow} + \k{\downarrow\uparrow})\\
    \k{1,1}     &= \k{\uparrow\uparrow}.
\end{align}
...
We now consider two angular-momentum operators $\vec{J}_1$ and $\vec{J}_2$, living in different subspaces. The individual components of each operator satisfies the normal commutation relations, but they commute with the components of the other operator. Looking at the infinitesimal rotation of both subspaces simultaneously, we see
\begin{equation}
    \ab (1 - i \frac{\vec{J}_1 \cdot \hat{\vec{n}}}{\hbar} \delta \phi) \otimes \ab (1 - i \frac{\vec{J}_2 \cdot \hat{\vec{n}}}{\hbar} \delta \phi) = 1 - i \frac{(\vec{J}_1 \otimes 1 + 1 \otimes \vec{J}_2)\cdot \hat{\vec{n}}}{\hbar} \delta\phi.
\end{equation} 
We define the total angular momentum operator as
\begin{equation}
    \vec{J} = \vec{J}_1 \otimes 1 + 1 \otimes \vec{J}_2 = \vec{J}_1 + \vec{J}_2.
\end{equation}
For a finite rotation we have
\begin{equation}
    \mathscr{D}_1(R) \otimes \mathscr{D}_2(R) = \exp\ab(-i \frac{\vec{J}_1 \cdot \hat{\vec{n}}}{\hbar} \phi) \otimes \exp\ab(-i \frac{\vec{J}_2 \cdot \hat{\vec{n}}}{\hbar} \phi).
\end{equation}
Note the apparent similarities of the angle and axis of rotation for both subspaces. Importantly, the components of $\vec{J}$ satisfy the commutation relation for angular momentum. Physically the total angular momentum operator is the generator of rotation for the entire system. For choosing base kets there are two options:
\begin{enumerate}
    \item The simultaneous eigenkets of $\vec{J}_1^2, \vec{J}_2^2, J_{1z},$ and $J_{2z}$, or the $\{j_1,j_2,m_1,m_2\}$ basis, denoted $\k{j_1,j_2;m_1,m_2}$. The defining equations are
        \begin{alphaAlign}
            \vec{J}_1^2 \k{j_1,j_2;m_1,m_2} &= j_1(j_1 + 1)\hbar^2 \k{j_1,j_2;m_1,m_2}\\
            \vec{J}_2^2 \k{j_1,j_2;m_1,m_2} &= j_2(j_2 + 1)\hbar^2 \k{j_1,j_2;m_1,m_2}\\
            J_{1z} \k{j_1,j_2;m_1,m_2} &= m_1\hbar \k{j_1,j_2;m_1,m_2}\\
            J_{2z} \k{j_1,j_2;m_1,m_2} &= m_2 \hbar \k{j_1,j_2;m_1,m_2}
        \end{alphaAlign} 
    \item The simultaneous eigenkets of $\vec{J}_1^2, \vec{J}_2^2, \vec{J}^2$, and $J_z$ or the $\{j_1, j_2; j, m\}$ basis. We note that this set commutes, denoted $\k{j_1, j_2; j, m}$. The defining equation are
        \begin{alphaAlign}
            \vec{J}_1^2 \k{j_1, j_2; j, m} &= j_1(j_1 + 1)\hbar^2 \k{j_1, j_2; j, m}\\
            \vec{J}_2^2 \k{j_1, j_2; j, m} &= j_2(j_2 + 1)\hbar^2 \k{j_1, j_2; j, m}\\
            \vec{J}^2 \k{j_1, j_2; j, m} &= j(j + 1)\hbar^2 \k{j_1, j_2; j, m}\\
            J_z \k{j_1, j_2; j, m} &= m\hbar \k{j_1, j_2; j, m}.
        \end{alphaAlign}
    Commonly the values for $j_1,j_2$ are implicit, and the ket is denoted as just $\k{j,m}$. 
\end{enumerate}
It is important to note that
\begin{equation}
    \comm{\vec{J}^2}{J_z} = 0, \quad \comm{\vec{J}^2}{J_{1z}} \neq 0, \quad \comm{\vec{J}^2}{J_{2z}} \neq 0.
\end{equation}
We can therefore not mix these two representations. Consider the connecting transformation for the two bases
\begin{equation}
    \k{j_1, j_2; j, m} = \sum_{m_1} \sum_{m_2} \k{j_1, j_2; m_1, m_2} \bk{j_1, j_2; m_1, m_2}{j_1, j_2; j, m}.
\end{equation}
The elements $\bk{j_1, j_2; m_1, m_2}{j_1, j_2; j, m}$ are the Clebsch-Gordan coefficients.
...
\begin{equation}
    \bk{j_1, j_2; m_1, m_2}{j_1, j_2; j, m} = (-1)^{j_1 - j_2 + m} \sqrt{2j + 1}
    \begin{pmatrix}
        j_1 & j_2 & j\\
        m_1 & m_2 & -m
    \end{pmatrix}.
